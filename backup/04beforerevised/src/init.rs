...
use std::io::prelude::*;
use std::io;
use crate::util::{command, get_files, send_to_file, get_home_dir, hex_to_rgb};
use dirs::home_dir;
...
pub fn get_args() -> Args {
    let matches = App::new("walmate")
        ...
        .arg(Arg::with_name("type")
             ...
             .required(true)
             .value_name("TYPE")
             .help("wether it should be light or dark"))
    ...
    let display = match matches.value_of("type") {
        Some(x) => {
            match x {
                "dark" | "light" => x.to_string(),
                _ => panic!("unknown display type")
            }
        },
        None => panic!("A display type is required (dark/light)")
    };
...
pub fn get_colors(args: &Args, variables: &Variables, files: &Vec<String>) -> Vec<(String, (String, String, String))> {
    // generate a filename
    ...
    match &args.input {
        ...
        _ => {
            ...
            if base16.contains("rand") {
                let rng: f64 = rand::thread_rng().gen();
                selected = files[(rng * files.len() as f64) as usize].to_owned()
            } else {
                for file in &files {
                    let file = file.to_owned();
                    // println!("file is {}, filename is {}", file, filename);
    ...
    // now we are going to generate rgb values too.
    let mut rgb: Vec<(String, (String, String, String))> = Vec::new();
    for color in &colors_separated {
        let converted = hex_to_rgb(color);
        rgb.push((color.to_owned(), converted))
    }
    rgb
}
